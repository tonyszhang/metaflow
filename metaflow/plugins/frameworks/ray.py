...
class RayParallelDecorator(ParallelDecorator):
    ...
    def setup_distributed_env(self, flow, ubf_context):
        ray_cli_path = sys.executable.replace("python", "ray") # assumes ray is installed in the environment via user code or base image. 
...
def setup_ray_distributed(
    ...
    # https://github.com/Netflix/metaflow/blob/76eee802cba1983dffe7e7731dd8e31e2992e59b/metaflow/task.py#L553
        # this causes the following current.parallel variables to be defaults on all nodes here,
    ...
    # Workaround that doesn't generalize to non-AWS compute providers.
